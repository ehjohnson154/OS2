...
//     inode_alloc->block_ptr = 0; //does NOT
...
//     for (i=0; i < MAX_SYS_OPEN_FILES; i++){//CHANGE TO FOR LOOP
//         struct inode * check = incore[i];
...
//     unsigned char block = bread(block_num, empty_block);
...
//     for (i=0; i < 16; i++){//    //does this work to search through block pointers? //probably not?
...
//     for (i=0; i < 16; i++){    //sets everything to empty block?
