...
void mkfs(void){
   ...
   // inode numbers are the same as the root inode itself 
    ...
    write_u16(root_0, inode_num); //what is the addr? block[0]? //.
    //what is the second directory
    write_u16(root_1, inode_num); //..
    ...
    unsigned char *root_1_name = root_1+2;
...
struct directory *directory_open(int inode_num){ 
    ...
    struct directory *directory_stuff; // HOW TO INITIALIZE DIRECTORY?
    directory_stuff = malloc(sizeof(*directory_stuff)); //malloc returns pointer to where the space is.
...
int directory_get(struct directory *dir, struct directory_entry *ent){
    ...
    ent->inode_num = read_u16(dir_inode);
...
int directory_make(char *path){
    ...
    char *parent = malloc(sizeof(*parent));

    get_dirname(path, parent);
    ...
    write_u16(current_dir, new_inode_num); //. current //
    
    write_u16(parent_dir, parent_inode_num); //.. parent
